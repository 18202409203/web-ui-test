// Version 2.6.3 timelines-chart - https://github.com/vasturiano/timelines-chart
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
	typeof define === 'function' && define.amd ? define(factory) :
	(global.TimelinesChart = factory());
...
function conicConformalRaw(y0, y1) {
  ...
  project.invert = function(x, y) {
    ...
    return [atan2(x, abs(fy)) / n * sign(fy), 2 * atan(pow$1(f / r, 1 / n)) - halfPi$2];
  };
...
function bimap$1(domain, range$$1, deinterpolate, reinterpolate$$1) {
  var d0 = domain[0], d1 = domain[1], r0 = range$$1[0], r1 = range$$1[1];
  if (d1 < d0) d0 = deinterpolate(d1, d0), r0 = reinterpolate$$1(r1, r0);
  else d0 = deinterpolate(d0, d1), r0 = reinterpolate$$1(r0, r1);
  return function(x) { return r0(d0(x)); };
...
function polymap$1(domain, range$$1, deinterpolate, reinterpolate$$1) {
  var j = Math.min(domain.length, range$$1.length) - 1,
      d = new Array(j),
      r = new Array(j),
      i = -1;
...
function log$2() {
  ...
  scale.copy = function() {
    return copy$1(scale, log$2().base(base));
  };
...
function calendar$1(year$$1, month$$1, week, day$$1, hour$$1, minute$$1, second$$1, millisecond$$1, format) {
  ...
  var tickIntervals = [
    ...
    [minute$$1,  1,      durationMinute$2],
    [minute$$1,  5,  5 * durationMinute$2],
    [minute$$1, 15, 15 * durationMinute$2],
    [minute$$1, 30, 30 * durationMinute$2],
    [  hour$$1,  1,      durationHour$2  ],
    [  hour$$1,  3,  3 * durationHour$2  ],
    [  hour$$1,  6,  6 * durationHour$2  ],
    [  hour$$1, 12, 12 * durationHour$2  ],
    [   day$$1,  1,      durationDay$2   ],
    [   day$$1,  2,  2 * durationDay$2   ],
    [  week,  1,      durationWeek$2  ],
    [ month$$1,  1,      durationMonth$1 ],
    [ month$$1,  3,  3 * durationMonth$1 ],
    [  year$$1,  1,      durationYear$1  ]
  ];
  ...
  scale.domain = function(_) {
    return arguments.length ? domain(map$3.call(_, number$5)) : domain().map(date$2);
  };
  ...
  scale.copy = function() {
    return copy$1(scale, calendar$1(year$$1, month$$1, week, day$$1, hour$$1, minute$$1, second$$1, millisecond$$1, format));
  };
...
function time() {
  return calendar$1(year, month, sunday, day, hour, minute, second, millisecond, timeFormat).domain([new Date(2000, 0, 1), new Date(2000, 0, 2)]);
}

function utcTime() {
  return calendar$1(utcYear, utcMonth, utcSunday, utcDay, utcHour, utcMinute, second, millisecond, utcFormat).domain([Date.UTC(2000, 0, 1), Date.UTC(2000, 0, 2)]);
}
...
var cubehelix$4 = cubehelixLong(cubehelix(300, 0.5, 0.0), cubehelix(-240, 0.5, 1.0));
...
var _d3_4_13_0_d3 = Object.freeze({
	version: version,
	bisect: bisectRight,
...
var _d3Tip_0_7_1_d3Tip = createCommonjsModule(function (module) {
// d3.tip
...
var gradient = Kapsule({
    props: {
        ...
        colorScale: { default: linear$3().range(['black', 'white']) },
    ...
    update: function update(state) {
        ...
        var stopsScale = linear$3().domain([0, 100]).range(state.colorScale.domain());
...
var TimeOverview = Kapsule({
  ...
  init: function init(el, state) {
    ...
    state.brush = brushX().on('end', function () {
      ...
      var selection$$1 = event.selection ? event.selection.map(state.scale.invert) : state.scale.domain();
      state.onChange.apply(state, toConsumableArray$1(selection$$1));
    });
...
var timelines = Kapsule({
  props: {
    data: {
      ...
      onChange: function onChange(data, state) {
        ...
        }), d3Max(state.completeFlatData, function (d) {
    ...
    zColorScale: { default:
      ...
      ordinal() // 按顺序依次对应 停歇原因 和 颜色
      .domain(['0001', '0002', '0003', '0004', '0005', '0006', '0007', '0008', '0009', '0010', '0011', '0012', '0013', '0014', '0015', '0016', '0017', '0018', '0019', '0020', '1000']).range([].concat(toConsumableArray$1(schemeCategory10), toConsumableArray$1(schemeSet3)))
    },
  ...
  init: function init(el, state) {
    ...
    function addTooltips() {
      state.groupTooltip = _d3Tip_0_7_1_d3Tip().attr('class', 'chart-tooltip group-tooltip').direction('w').offset([0, 0]).html(function (d) {
      ...
      state.lineTooltip = _d3Tip_0_7_1_d3Tip().attr('class', 'chart-tooltip line-tooltip').direction('e').offset([0, 0]).html(function (d) {
      ...
      state.segmentTooltip = _d3Tip_0_7_1_d3Tip().attr('class', 'chart-tooltip segment-tooltip').direction('s').offset([5, 0]).html(function (d) {
    ...
    function setEvents() {
      ...
      state.svg.on('resetZoom', function () {
        ...
        var newZoomX = state.enableOverview ? state.overviewArea.domainRange() : [min(state.flatData, function (d) {
          return d.timeRange[0];
        }), d3Max(state.flatData, function (d) {
          return d.timeRange[1];
        })],
            newZoomY = [null, null];
  ...
  update: function update(state) {
    ...
    function adjustXScale() {
      ...
      state.zoomX[1] = state.zoomX[1] || d3Max(state.flatData, function (d) {
        return d.timeRange[1];
      });
    ...
    function renderAxises() {
      ...
      if (state.onLabelClick) {
        state.svg.selectAll('g.y-axis,g.grp-axis').selectAll('text').style('cursor', 'pointer').on('click', function (d) {
          var segms = d.split('+&+');
          state.onLabelClick.apply(state, toConsumableArray$1(segms.reverse()));
        });
      }
