...
export default {
    ...
    methods:{
        fetchData() {
            let vm = this
            axios.get(this.ip + '/lyj/stoptime/getEquipTimeLineByDateScopAndProcedureid/2017-1-1/2017-2-1/all').then(response => {
                    ...
                    TimelinesChart()
                        ...
                        .zColorScale(
                            d3.scaleOrdinal() // 按顺序依次对应 停歇原因 和 颜色
                                .domain(function(){
                                    return vm.categoryLabels.map( (d, i)=> d.stopName)
                                }())
                                .range(["#2962FF",...d3.schemeBlues[7].slice(2),...d3.schemeGreens[8].slice(2),"#42A5F5",
                                "#FFA726","#FF9100",...d3.schemeReds[9].slice(3)])
                        )
                        //********************************************//
